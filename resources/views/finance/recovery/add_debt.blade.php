...
<div class="bg-white rounded-2xl shadow-sm p-6 max-w-2xl">
    ...
    <form method="POST" action="{{ route('recovery.add-debt.store') }}">
        ...
        <button type="submit" class="mt-6 px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl font-medium">
            Add Debit Entry
        </button>
    </form>
    {{-- Check Invoice --}}
    <div class="mt-8 border-t pt-6">
        ...
        <form method="GET" action="{{ route('recovery.add-debt') }}" class="flex gap-3">
            ...
            <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm">Load</button>
        </form>
    </div>
</div>
@endsection
