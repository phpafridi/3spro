    ...
    <form method="POST" action="{{ route('recovery.followup', ['id' => $id]) }}" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="remarks" rows="3"
                      ...
                      class="w-full border border-gray-300 rounded px-4 py-2 focus:ring-2 focus:ring-green-400 text-sm"></textarea>
        </div>
        <div class="flex gap-3">
            <button type="submit" class="px-6 py-2 bg-green-600 text-white rounded text-sm font-medium hover:bg-green-700 transition">
                <i class="fas fa-save mr-2"></i>Save Followup
            </button>
            <a href="{{ route('recovery.history', ['id' => $id]) }}"
               class="px-6 py-2 bg-gray-100 text-gray-700 rounded text-sm font-medium hover:bg-gray-200 transition">
                View History
            </a>
        </div>
    </form>
