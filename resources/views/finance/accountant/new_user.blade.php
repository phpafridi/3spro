...
<div class="bg-white rounded-2xl shadow-sm p-6 max-w-2xl">
    <h2 class="text-2xl font-semibold text-gray-800 mb-6">
        <i class="fas fa-user-plus text-indigo-500 mr-2"></i> Create New User
    </h2>
    <form method="POST" action="{{ route('accountant.new-user.store') }}" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="login_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Password</label>
                <input type="password" name="password2" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            </div>
            <div>
            ...
            <div>
                ...
                <select name="position" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                    <option value="">-- Select --</option>
                    @foreach(['Finance-FManager','Finance-Accountant','Finance-Cashier','Finance-RecoveryExec','Service-SerAdvisor','Service-SManager','Service-JobController','Parts-PManager','Parts-DataOperator','IT-IT Manager'] as $opt)
