...
<div class="bg-white rounded shadow-sm p-6">
    <h2 class="text-2xl font-semibold text-gray-800 mb-6">
        <i class="fas fa-clipboard-list text-red-500 mr-2"></i> Labor Requests
    </h2>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-3 py-3 flex gap-2">
                        <form method="POST" action="{{ route('accountant.labor-request.process') }}">
                            ...
                            <button class="px-3 py-1 bg-green-500 hover:bg-green-600 text-white text-xs rounded">Accept</button>
                        </form>
                        <form method="POST" action="{{ route('accountant.labor-request.process') }}">
                            @csrf
                            <input type="hidden" name="req_id" value="{{ $r->req_id }}">
