...
.report-title { background: linear-gradient(135deg, #0284c7 0%, #0369a1 100%); color: #fff; }
</style>
</head>
<body>
<div class="page-wrap">
  ...
  <div class="report-title">
    <div>
      <h2>Cash Flow — GSL Detail</h2>
      <div class="period">Period: {{ $from }} &nbsp;–&nbsp; {{ $to }}</div>
    </div>
    <div class="badge">{{ $glName ?? '' }}</div>
  </div>

  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px;">
    @php $totIn=collect($rows)->sum('totalCredit'); $totOut=collect($rows)->sum('totalDebit'); @endphp
    <div style="background:#fff;border-radius:8px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,.07);border-top:3px solid #16a34a;">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;color:#888;">Total Inflow</div>
      <div style="font-size:17px;font-weight:700;font-family:monospace;color:#16a34a;">{{ number_format($totIn,2) }}</div>
    </div>
    <div style="background:#fff;border-radius:8px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,.07);border-top:3px solid #dc2626;">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;color:#888;">Total Outflow</div>
      <div style="font-size:17px;font-weight:700;font-family:monospace;color:#dc2626;">{{ number_format($totOut,2) }}</div>
    </div>
    <div style="background:#fff;border-radius:8px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,.07);border-top:3px solid {{ $netClosing >= 0 ? '#2563eb' : '#dc2626' }};">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;color:#888;">Net Position</div>
      <div style="font-size:17px;font-weight:700;font-family:monospace;color:{{ $netClosing >= 0 ? '#2563eb' : '#dc2626' }};">{{ number_format($netClosing,2) }}</div>
    </div>
  </div>
  ...
  <table class="rpt-table" id="myTable">
    <thead>
      <tr>
        ...
        <th class="num">Inflow (Cr)</th><th class="num">Outflow (Dr)</th><th class="num">Net</th>
      </tr>
    </thead>
    <tbody>
    @foreach($rows as $i => $row)
    @php $net = $row->totalCredit - $row->totalDebit; @endphp
    <tr>
      <td style="color:#9ca3af;font-size:12px;">{{ $i+1 }}</td>
      <td style="font-family:monospace;font-size:12px;color:#7c3aed;font-weight:600;">{{ $row->GSL_code }}</td>
      <td>{{ $row->GSL_name }}</td>
    ...
    <tr>
      ...
      <td class="num">{{ number_format($totIn,2) }}</td>
      <td class="num">{{ number_format($totOut,2) }}</td>
      <td class="num {{ $netClosing >= 0 ? 'amt-pos' : 'amt-neg' }}">{{ number_format($netClosing,2) }}</td>
    </tr>
    </tfoot>
  </table>
</div>
