...
.report-title { background: linear-gradient(135deg, #0891b2 0%, #0e7490 100%); color: #fff; }
</style>
</head>
<body>
<div class="page-wrap">
  ...
  <table class="rpt-table" id="myTable">
    <thead>
      <tr>
        ...
        <th class="num">Receipts (In)</th>
        <th class="num">Payments (Out)</th>
    ...
    <tr>
      ...
      <td class="num" style="color:#555;">{{ number_format($row->opening,2) }}</td>
      <td class="num amt-cr">{{ number_format($row->totalCredit,2) }}</td>
      <td class="num amt-dr">{{ number_format($row->totalDebit,2) }}</td>
      <td class="num {{ $closing >= 0 ? 'amt-pos' : 'amt-neg' }}">{{ number_format($closing,2) }}</td>
    </tr>
    @endforeach
    </tbody>
    <tfoot>
    @php
      $totOpen  = collect($rows)->sum('opening');
      $totIn    = collect($rows)->sum('totalCredit');
      $totOut   = collect($rows)->sum('totalDebit');
      $totClose = $totOpen + $totIn - $totOut;
    @endphp
    <tr>
      <td colspan="2">Grand Total</td>
      <td class="num">{{ number_format($totOpen,2) }}</td>
      <td class="num">{{ number_format($totIn,2) }}</td>
      <td class="num">{{ number_format($totOut,2) }}</td>
      <td class="num">{{ number_format($totClose,2) }}</td>
    </tr>
    </tfoot>
  </table>
</div>
