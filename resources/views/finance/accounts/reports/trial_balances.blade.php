...
.report-title { background: linear-gradient(135deg, #16a34a 0%, #15803d 100%); color: #fff; }
</style>
</head>
<body>
<div class="page-wrap">
  ...
  <div class="report-title">
    <div>
      <h2>GL Trial Balances</h2>
      <div class="period">As on {{ $asOn }}</div>
    </div>
    <div class="badge">&#9878; Trial Balance</div>
  </div>
  ...
  <table class="rpt-table" id="myTable">
    ...
    <tr>
      ...
      <td class="num {{ $row->TrialBalance >= 0 ? 'amt-pos' : 'amt-neg' }}">{{ number_format($row->TrialBalance,2) }}</td>
    </tr>
    @endforeach
    </tbody>
    <tfoot>
    @php
      $totDr  = collect($rows)->sum('Debit');
      $totCr  = collect($rows)->sum('Credit');
      $totBal = $totDr - $totCr;
    @endphp
    <tr>
      <td colspan="3">Grand Total</td>
      <td class="num">{{ number_format($totDr,2) }}</td>
      <td class="num">{{ number_format($totCr,2) }}</td>
      <td class="num">{{ number_format($totBal,2) }}</td>
    </tr>
    </tfoot>
  </table>
</div>
