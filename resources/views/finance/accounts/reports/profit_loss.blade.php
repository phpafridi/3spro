...
.report-title { background: linear-gradient(135deg, #059669 0%, #047857 100%); color: #fff; }
.section-pill { display:inline-flex;align-items:center;gap:6px;padding:5px 14px;border-radius:20px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.5px; }
.pill-inc { background:#dcfce7;color:#166534; } .pill-exp { background:#fee2e2;color:#991b1b; }
.section-row { display:flex;align-items:center;gap:10px;margin:20px 0 10px; }
.section-row .line { flex:1;height:1px;background:#e5e7eb; }
</style>
</head>
<body>
<div class="page-wrap">
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px;">
    <div style="background:#fff;border-radius:8px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,.07);border-top:3px solid #16a34a;">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;color:#888;">Total Income</div>
      <div style="font-size:17px;font-weight:700;font-family:monospace;color:#16a34a;">{{ number_format($totalIncome,2) }}</div>
    </div>
    <div style="background:#fff;border-radius:8px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,.07);border-top:3px solid #dc2626;">
      ...
      <div style="font-size:17px;font-weight:700;font-family:monospace;color:#dc2626;">{{ number_format($totalExpense,2) }}</div>
    </div>
    <div style="background:#fff;border-radius:8px;padding:12px 16px;box-shadow:0 1px 4px rgba(0,0,0,.07);border-top:3px solid {{ $net >= 0 ? '#2563eb' : '#dc2626' }};">
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;color:#888;">Net {{ $net >= 0 ? 'Profit' : 'Loss' }}</div>
      <div style="font-size:17px;font-weight:700;font-family:monospace;color:{{ $net >= 0 ? '#2563eb' : '#dc2626' }};">{{ number_format(abs($net),2) }}</div>
    </div>
  </div>

  <div class="toolbar"><button class="btn-act" onclick="window.print()">&#128438; Print</button></div>

  <div class="section-row"><span class="section-pill pill-inc">&#9650; Income</span><div class="line"></div><span style="font-weight:700;color:#16a34a;font-size:14px;">{{ number_format($totalIncome,2) }}</span></div>
  <table class="rpt-table" id="t1">
    <thead><tr><th>#</th><th>GSL Code</th><th>Account Name</th><th class="num">Amount</th></tr></thead>
    <tbody>
    @foreach($income as $i => $r)
    <tr>
      <td style="color:#9ca3af;font-size:12px;">{{ $i+1 }}</td>
      <td style="font-family:monospace;font-size:12px;color:#7c3aed;font-weight:600;">{{ $r->GSL_code }}</td>
      <td>{{ $r->GSL_name }}</td>
      <td class="num amt-cr">{{ number_format($r->TotalIncome,2) }}</td>
    </tr>
    @endforeach
    </tbody>
    <tfoot><tr><td colspan="3">Total Income</td><td class="num amt-cr">{{ number_format($totalIncome,2) }}</td></tr></tfoot>
  </table>

  <div class="section-row"><span class="section-pill pill-exp">&#9660; Expenses</span><div class="line"></div><span style="font-weight:700;color:#dc2626;font-size:14px;">{{ number_format($totalExpense,2) }}</span></div>
  <table class="rpt-table" id="t2">
    <thead><tr><th>#</th><th>GSL Code</th><th>Account Name</th><th class="num">Amount</th></tr></thead>
    <tbody>
    @foreach($expenses as $i => $r)
    <tr>
      <td style="color:#9ca3af;font-size:12px;">{{ $i+1 }}</td>
      <td style="font-family:monospace;font-size:12px;color:#7c3aed;font-weight:600;">{{ $r->GSL_code }}</td>
      <td>{{ $r->GSL_name }}</td>
      <td class="num amt-dr">{{ number_format($r->TotalExpense,2) }}</td>
    </tr>
    @endforeach
    </tbody>
    <tfoot><tr><td colspan="3">Total Expenses</td><td class="num amt-dr">{{ number_format($totalExpense,2) }}</td></tr></tfoot>
  </table>
  ...
  <div style="background:{{ $net >= 0 ? '#f0fdf4' : '#fef2f2' }};border:2px solid {{ $net >= 0 ? '#16a34a' : '#dc2626' }};border-radius:10px;padding:16px 24px;display:flex;justify-content:space-between;align-items:center;margin-top:10px;">
    <div style="font-size:14px;font-weight:700;color:#374151;">Net {{ $net >= 0 ? 'Profit' : 'Loss' }}</div>
    <div style="font-size:26px;font-weight:700;color:{{ $net >= 0 ? '#16a34a' : '#dc2626' }};font-family:monospace;">{{ $net >= 0 ? '' : '(' }}{{ number_format(abs($net),2) }}{{ $net >= 0 ? '' : ')' }}</div>
  </div>
</div>
...
<script>
$(function(){ $('#t1,#t2').DataTable({paging:false,dom:'Bfrtip',buttons:[{extend:'excel',text:'&#128196; Excel'},{extend:'csv',text:'CSV'},{extend:'print',text:'&#128438; Print'}]}); });
</script>
</body></html>
