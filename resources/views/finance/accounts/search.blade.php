...
<div class="bg-white rounded shadow-sm p-6">
    <h2 class="text-2xl font-semibold text-gray-800 mb-6">
        <i class="fas fa-search text-red-500 mr-2"></i>Search Voucher
    </h2>
    ...
    <form method="POST" action="{{ route('accounts.search') }}" class="flex gap-3 mb-8 max-w-md">
        ...
        <input type="text" name="search_voucher" value="{{ $voucher->mas_vch_id ?? '' }}"
               ...
               class="flex-1 border border-gray-300 rounded px-4 py-2 focus:ring-2 focus:ring-red-500">
    ...
    @if($voucher)
    {{-- Voucher Details --}}
    <div class="p-5 bg-red-50 border border-red-200 rounded mb-6">
    ...
    <div class="p-4 bg-yellow-50 border border-yellow-200 rounded">
        ...
        <form method="POST" action="{{ route('accounts.search') }}" class="space-y-3">
            ...
            <textarea name="reason" rows="2" required placeholder="State the reason for reopening..."
                      class="w-full border border-yellow-300 rounded px-4 py-2 text-sm focus:ring-2 focus:ring-yellow-400"></textarea>
            <button type="submit" class="px-5 py-2 bg-yellow-500 text-white rounded text-sm font-medium hover:bg-yellow-600">
