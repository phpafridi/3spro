...
<div class="grid grid-cols-1 md:grid-cols-5 gap-6">
    <div class="md:col-span-2 bg-white rounded-lg shadow-sm p-6">
        ...
        <form method="POST" action="{{ route('bp-jc.additional.part.store') }}" class="space-y-3">
            @csrf
            <input type="hidden" name="job_id" value="{{ $jobId }}">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Part Description <span class="text-red-500">*</span></label>
                <input type="text" name="part_description" required autocomplete="off" placeholder="Search part name..."
    ...
    <div class="md:col-span-3 bg-white rounded-lg shadow-sm p-6">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                @empty
                <tr><td colspan="5" class="px-4 py-4 text-center text-gray-400 text-sm italic">No parts yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
