...
@section('title', 'Estimate Sublet — #' . $estmId)
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <form method="POST" action="{{ route('jobcard.estimate.sublet.store') }}" class="space-y-3">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Sublet <span class="text-red-500">*</span></label>
                <input type="text" name="sublet" required style="text-transform:uppercase"
                       class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Type</label>
                <select name="type" id="sublet_type" onchange="toggleSubletPrice(this)"
                        class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="Workshop">Workshop</option>
                    <option value="Customer">Customer</option>
                </select>
            </div>
            <div class="grid grid-cols-3 gap-2" id="price_fields">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Qty</label>
                    <input type="number" name="qty" id="qty" min="1" value="1" oninput="calculate()"
                           class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Unit Price</label>
                    <input type="number" name="unitprice" id="unitprice" min="0" step="0.01" oninput="calculate()"
                           class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
...
function toggleSubletPrice(sel) {
    // Workshop = price fields visible, Customer = price fields hidden (0)
    var show = sel.value === 'Workshop';
    document.getElementById('price_fields').style.display = show ? '' : 'none';
    if (!show) {
...
document.querySelectorAll('.delete-btn').forEach(function (btn) {
    btn.addEventListener('click', function () {
        ...
        $.ajax({
            ...
            data: { _token: '{{ csrf_token() }}', sid: this.dataset.sid },
