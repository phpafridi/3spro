...
<div class="grid grid-cols-1 md:grid-cols-5 gap-6">
    <div class="md:col-span-2 bg-white rounded-lg shadow-sm p-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-4">Add Vendor</h2>
        <form method="POST" action="{{ route('sm.vendors.store') }}" class="space-y-3">
            @csrf
            @foreach(['jobber'=>'Vendor Name *','contactperson'=>'Contact Person','contact'=>'Contact','address'=>'Address','worktype'=>'Work Type'] as $name=>$label)
            <div>
    ...
    <div class="md:col-span-3 bg-white rounded-lg shadow-sm p-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-4">Vendors
            <span class="ml-2 px-2 py-0.5 bg-gray-100 text-gray-600 text-sm rounded-full">{{ $vendors->count() }}</span>
        </h2>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Work Type</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-3 py-2 text-sm font-medium text-gray-800">{{ $v->vendor_name }}</td>
                        <td class="px-3 py-2 text-sm text-gray-500">{{ $v->contact }}</td>
                        <td class="px-3 py-2 text-sm text-gray-500">{{ $v->work_type }}</td>
                        ...
                        <td class="px-3 py-2">
                            <form method="POST" action="{{ route('sm.vendors.toggle') }}" class="inline">
                                @csrf
                    ...
                    @empty
                    <tr><td colspan="5" class="px-4 py-6 text-center text-gray-400">No vendors.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
