...
@section('title', 'Sales - Sublet Status')
@section('sidebar-menu') @include('sales.partials.sidebar') @endsection
@section('content')
<div class="flex justify-between items-center mb-4">
    <h2 class="text-xl font-semibold text-gray-800">Sublet Status — In Workshop</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 text-sm text-gray-400">{{ $s->entry_datetime }}</td>
                        <td class="px-4 py-2 text-sm text-gray-400">{{ $s->end_time ?? '—' }}</td>
                        <td class="px-4 py-2 text-sm">
                            <span class="px-2 py-0.5 bg-yellow-100 text-yellow-700 text-xs rounded-full">{{ $s->status ?: 'Pending' }}</span>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
